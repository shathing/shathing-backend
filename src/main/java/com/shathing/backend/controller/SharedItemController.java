...
public class SharedItemController {
    ...
    @PreAuthorize("isAuthenticated()")
    @PutMapping("/share/posts/{id}")
    public ResponseEntity<SharedItemResponse> updateSharedItem(
            @AuthenticationPrincipal Long memberId,
            @PathVariable Long id,
            @Valid @RequestBody UpdateSharedItemRequest request
    ) {
        return ResponseEntity.ok(sharedItemService.updateSharedItem(memberId, id, request));
    }

    @PreAuthorize("isAuthenticated()")
    @DeleteMapping("/share/posts/{id}")
    public ResponseEntity<Void> deleteSharedItem(
            @AuthenticationPrincipal Long memberId,
            @PathVariable Long id
    ) {
        sharedItemService.deleteSharedItem(memberId, id);
    ...
    public ResponseEntity<PageResponse<SharedItemResponse>> getSharedItems(
        ...
        return ResponseEntity.ok(sharedItemService.getSharedItems(categoryId, regionId, countryCode, search, page, size));
    }
